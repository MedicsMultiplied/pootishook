...
void ProcessUserCmd(CUserCmd *cmd)
{
	...
	case 14: // Mercedes
        ...
        case 15: // VIN DIESEL
        {
            cur_yaw[yaw_mode] += yaw_mode ? (float)150.0f * 50.0f: -((float)150.0f * 50.0f);
	    while (cur_yaw[yaw_mode] > 180.0f) cur_yaw[yaw_mode] += -360.0f;
	    while (cur_yaw[yaw_mode] < -180.0f) cur_yaw[yaw_mode] += 360.0f;
	    y = cur_yaw[yaw_mode];
        }
        break;
    ...
    case 6: // POOTIS
        {
            float angle = p * 25; angle = angle * 4;
            if (angle < 360.0f) angle = -360.0f;
            if (angle > 360.0f) angle = 360.0f;
            float resval = (M_PI + angle) - 90.0f;
            if (resval > 360.0f) resval = resval - (90.0f * 2);
            p = resval;
        }
        break;
    }
