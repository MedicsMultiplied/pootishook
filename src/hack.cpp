...
{
    static std::string version("Version: 1.0");
    static bool version_set = false;
    if (version_set)
        return version;
#if defined(GIT_COMMIT_HASH) && defined(GIT_COMMITTER_DATE)
    version = "u silly goose there is no git commit/committer hash/date";
#endif
    version_set = true;
    return version;
}
...
void critical_error_handler(int signum)
{
    ...
    for (auto i : st::stacktrace())
    {
        ...
        {
            unsigned int offset = reinterpret_cast<uintptr_t>(i.address()) - reinterpret_cast<uintptr_t>(info2.dli_fbase);
...
void hack::Hook()
{
    ...
    // FIXME [MP]?
    while (!(clientMode = **(uintptr_t ***) ((uintptr_t) ((*(void ***) g_IBaseClient)[11]) + 1)))
    {
        usleep(10000);
    }
