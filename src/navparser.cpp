...
static settings::Boolean fuckviewangles{ "nav.fviewangles", "false" };
static settings::Boolean looksmooth{ "nav.lapsmoothness", "true" };
...
static settings::Int PathColor{ "nav.pathcolor", "0" };
...
public:
    ...
    void AdjacentCost(void *main, std::vector<micropather::StateCost> *adjacent) override
    {
        ...
        for (NavConnect &connection : area.m_connections)
        {
            ...
            // If nojumppaths is enabled, skip paths that require jumping
...
static void followCrumbs()
{
    ...
    // Look at path
    if (fuckviewangles && !hacks::shared::aimbot::isAiming())
    {
        Vector next{ Vector(rand(), rand(), rand()).x, Vector(rand(), rand(), rand()).y, 0 };
        hacks::tf2::misc_aimbot::DoSlowAim(next);
        current_user_cmd->viewangles = next;
    }
